...
// sceJpegDecodeMJpegYCbCrSuccessively
extern "C" int sceJpeg_227662D7(const void *buffer, int bufferSize, void *frame, int frameSize, int dhtMode);
...
static void testJpegDecodeMJpegYCbCr(const char *title, const File &file, bool useFrame, int frameSize, int mode) {
	...
	uint32_t result = sceJpeg_227662D7(file.data, file.size, useFrame ? frame : NULL, frameSize, mode);
	size_t len = 0;
...
extern "C" int main(int argc, char *argv[]) {
	...
	checkpoint("  Offset: %08x", sceJpeg_227662D7(jpegcolor.data - 1, jpegcolor.size + 1, frame, FRAME_SIZE, 0));
