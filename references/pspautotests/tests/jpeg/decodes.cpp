...
// sceJpegDecodeMJpegSuccessively
extern "C" int sceJpeg_64B6F978(const void *buffer, int bufferSize, void *frame, int dhtMode);
...
static void testJpegDecodeMJpeg(const char *title, const File &file, bool useFrame, int mode) {
	...
	uint32_t result = sceJpeg_64B6F978(file.data, file.size, useFrame ? frame : NULL, mode);
	size_t len = 0;
...
extern "C" int main(int argc, char *argv[]) {
	...
	checkpoint("  Offset: %08x", sceJpeg_64B6F978(jpegcolor.data - 1, jpegcolor.size + 1, frame, 0));
