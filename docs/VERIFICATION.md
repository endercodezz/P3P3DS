...
| Total library import stubs: 221 across 22 libraries | `[VERIFIED]` | `experiments/p3p-analysis/compare_analysis.py` | Bit-exact 100% agreement between independent parser and PSPRecomp for all 221 tuples. |
...
| Deterministic execution milestone reached | `[VERIFIED]` | `build/p3p_pc_bootstrap.exe --verify-milestone` | Starts at `0x08804108`, halts on `Missing HLE import SysMemUserForUser::0x35669D4C` at PC `0x08B7FC0C`, $ra=`0x0880413C`. |
...
| All .text instructions recognized by baseline decoder | `[WRONG]` | Empirical decoder test (`psprecomp::decode_allegrex`) | 912,337 instructions (99.92%) recognized; 722 instructions (0.08%) currently unsupported in baseline (`madd`/`maddu`, `break`, traps). |
| Baseline decoder recognizes 99.92% of .text (912,337 / 913,059) | `[VERIFIED]` | Empirical decoder pass over `.text` using `verify_p3p_decoder` | Exactly 722 instructions unsupported (468 madd/maddu, 251 break, 3 traps). Consistent mutually exclusive sum: 468 + 251 + 3 = 722. |
