...
| Deterministic execution milestone reached | `[VERIFIED]` | `build/p3p_pc_bootstrap.exe --verify-milestone` | Starts at `0x08804108`, executes `SysMemUserForUser::0x35669D4C` (`sceKernelSetCompiledSdkVersion600_602`, SDK `0x06020010`), returns to `0x0880413C`, executes `SysMemUserForUser::0xF77D77CB` (`sceKernelSetCompilerVersion`, version `0x00030306`), returns to `0x08804148`, branches to out-of-line block `0x08804210`, jumps back to `0x08804188`, creates thread via `ThreadManForUser::0x446D8DE6` (`sceKernelCreateThread`), starts thread via `ThreadManForUser::0xF475845D` (`sceKernelStartThread`), context switches to `user_main` [UID 2] at `0x0880421C`, executes its prologue and initial blocks, and halts on call to uncompiled `sub_08B4E6A0` at PC `0x08B4E6A0`, $ra=`0x08804268`. |
